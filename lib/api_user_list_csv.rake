...
namespace :api_user_list_csv do
  ...
  task run: :environment do
    ...
    csv_file_path = Rails.root.join('public', 'user_data', "api_user_data_#{DateTime.now}.csv")
  ...
  def save_user_to_csv(user)
    ...
    user_data << user.id.to_s
    user_data << user.profile.first_name
    user_data << user.profile.last_name
    user_data << user.email
    user_data << user.profile_type
    user_data << user.profile_id.to_s
    user_data << user.created_at.to_date.to_s
    user_data << user.updated_at.to_date.to_s
